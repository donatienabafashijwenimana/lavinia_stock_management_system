...
<?php
include 'connect.php';
$select = $conn->query("select*from product,productin where pid=productid and  productid='{$_GET['pind']}'");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="table">
        <a href='?pin' class="close">&times</a>
        <h2><u>product out detail</u></h2>
       <?php if (mysqli_num_rows($select)>0){?>
        <table border="1">
            ...
            <tr>
                ...
                <td><?=$x['date']?></td>
                <td><?=$x['pname']?></td>
                <td><?=$x['quantity']-0?>kg</td>
                <td><a href="?pind&&up=<?=$x['stid']?>"><button>update</button></a></td>
                <td><a onclick="return confirm('are you sure you want to delete record of <?=$x['quantity']?>kg')"
                 href="delete.php?delpin=<?=$x['stid']?>&&qty=<?=$x['quantity']?>&&pid=<?=$x['pid']?>">
   ...
   <?php } else ?> <h1>!!no transction on this product</h1>
       ...
       <form action="add.php" method="post">
        <?php
        $select = $conn->query("select*from product,productin where pid=productid and  stid='{$_GET['up']}'");
        ...
        ?>
            ...
            <input type="submit" value="update product in" name="updatein" r>
        </form>
    </div>
    </div>
</body>
